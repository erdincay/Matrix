 ...
 * Time: 10:13 PM
 */
public class StrassenMatrixTestCorrectness {
    ...
    public void testProd() throws Exception {
        for (MatrixData md : mds_) {
            for (int i = 0; i < md.matrix_nums; i++) {
                StrassenMatrix left = new StrassenMatrix(md.line_size, md.row_size, md.left.get(i));
                StrassenMatrix right = new StrassenMatrix(md.line_size, md.row_size, md.right.get(i));
                StrassenMatrix ret = left.prod(right);
